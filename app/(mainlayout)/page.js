...
export default async function Home() {
   ...
   const movieResponse = await fetch(
      "https://firebasestorage.googleapis.com/v0/b/patron-movie-requests.appspot.com/o/movie-data-test.json?alt=media&token=616b93b4-c038-446e-a532-de691d4477c7"
   );
      ...
      <div className="flex flex-col justify-between p-[16px]">
         <div className="max-w-[1200px] w-full mx-auto">
            ...
            {/* {movieRequests.map((request) => (
               <div key={request._id}>{request.data.Title}</div>
            ))} */}
            <VotingApp
               user={user}
               ...
               readOnlyRequests={movieRequests}
            />
         </div>
      </div>
   );
